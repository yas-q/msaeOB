#' @title Parametric Bootstrap Mean Squared Error Estimators of Optimum Benchmarking for Univariate Non Sampled Area in Small Area Estimation
#'
#' @description Calculates the parametric bootstrap mean squared error estimates of optimum benchmarking for univariate non sampled area in small area estimation
#'
...
#' }
mse_saeOBns<-function (formula, vardir, weight, cluster, samevar = FALSE,
                       B = 100, MAXITER = 100, PRECISION = 1e-04, data)
{
  ...
  r = length(formula)
  if (r > 1)
    stop("You should use mse_msaeOBns() for multivariate")
  ...
  eblup_inside = function(r, n, samevar, y, X, cluster, vardir,
    ...
    df.vardir = data.frame(cluster[, 1], varians.direct[,
                                                        1])
    for (j in 1:n) {
  ...
  while (boot <= B) {
    ...
    random.effect.simula = as.matrix(random.effect.simula[,
                                                          -1])
    ...
    random.effect2.simula = as.matrix(random.effect2.simula[,
                                                            -1])
  ...
  msebootoptimum.df = as.data.frame(msebootoptimum.df)
  colnames(msebootoptimum.df) = y_names
  end_time = Sys.time()
